								...
								tensor* input_images, int base, int shuffle_index[])
{
	...
	__m256d delta_w_p_c0;
	__m256d delta_w_p_c1;
	__m256d delta_w_p_c2;
	__m256d delta_w_p_c3;
	__m256d delta_ws_p;
	...
	__m256d input_pixel_p_c0;
	__m256d input_pixel_p_c1;
	__m256d input_pixel_p_c2;
	__m256d input_pixel_p_c3;
	...
	__m256d del_conv_val_p_rem;
	__m256d input_pixel_p_rem;
	__m256d delta_w_p_rem;
    ...
    for (int f = 0; f < NUM_FILS; ++f)
	{
		...
		for (int r = 0; r < FIL_ROWS; ++r)
		{
			...
			for (c = 0; c+3 < FIL_COLS; c=c+4)
			{
			    ...
			    delta_w_p_c0  = _mm256_set1_pd(0.0);
			    delta_w_p_c1  = _mm256_set1_pd(0.0);
			    delta_w_p_c2  = _mm256_set1_pd(0.0);
			    delta_w_p_c3  = _mm256_set1_pd(0.0);
			    delta_w_p_rem = _mm256_set1_pd(0.0);
			    ...
			    for (int b = 0; b < BATCH_SIZE; ++b)
			    {
	    			...
	    			for (int i = 0; i < N_ROWS_CONV; ++i)
	    			{
	    				...
	    				for (j = 0; j+3 < N_COLS_CONV; j=j+4)
	    				{
	    					...
	    					input_pixel_p_c0  = _mm256_loadu_pd( (input_images->data) + ind_input_img(cur_image, i+r, j+c  ) );
	    					input_pixel_p_c1  = _mm256_loadu_pd( (input_images->data) + ind_input_img(cur_image, i+r, j+c+1) );
	    					input_pixel_p_c2  = _mm256_loadu_pd( (input_images->data) + ind_input_img(cur_image, i+r, j+c+2) );
	    					input_pixel_p_c3  = _mm256_loadu_pd( (input_images->data) + ind_input_img(cur_image, i+r, j+c+3) );


	    					// applying ReLU
	    					vmask = _mm256_cmp_pd(conv_val_p, zeroes_p, 0x0e); // 0x0e => GT. 
	    					and_p = _mm256_and_pd(del_conv_val_p, vmask); // zero out the del_convs for which convs are less than equal to zero

	    					mul_p_c0     = _mm256_mul_pd(and_p, input_pixel_p_c0);
	    					mul_p_c1     = _mm256_mul_pd(and_p, input_pixel_p_c1);
	    					mul_p_c2     = _mm256_mul_pd(and_p, input_pixel_p_c2);
	    					mul_p_c3     = _mm256_mul_pd(and_p, input_pixel_p_c3);

	    					delta_w_p_c0 = _mm256_add_pd(delta_w_p_c0, mul_p_c0);
	    					delta_w_p_c1 = _mm256_add_pd(delta_w_p_c1, mul_p_c1);
	    					delta_w_p_c2 = _mm256_add_pd(delta_w_p_c2, mul_p_c2);
	    					delta_w_p_c3 = _mm256_add_pd(delta_w_p_c3, mul_p_c3);
	    				}
	    				...
	    				for (; j < N_COLS_CONV; ++j)
	    				{
	    					...
	    					conv_val_rem = (conv_t->data)[ind_conv_out(b, f, i, j)];

	    					del_conv_val_p_rem  = _mm256_loadu_pd( (del_conv->data) + ind_conv_out(b, f, i, j) );
	    					input_pixel_p_rem   = _mm256_loadu_pd( (input_images->data) + ind_input_img(cur_image, i+r, j+c  ) );

	    					// derivative of ReLU
	    					if( conv_val_rem > 0.0 )
	    					{
	    						delta_w_p_rem += _mm256_mul_pd(del_conv_val_p_rem, input_pixel_p_rem);
	    					}
	    				}
	    			}
			    }
			    ...
			    delta_w_c0 = ( delta_w_p_c0[0] + delta_w_p_c0[1] + delta_w_p_c0[2] + delta_w_p_c0[3] + delta_w_p_rem[0] );
			    delta_w_c1 = ( delta_w_p_c1[0] + delta_w_p_c1[1] + delta_w_p_c1[2] + delta_w_p_c1[3] + delta_w_p_rem[1] );
			    delta_w_c2 = ( delta_w_p_c2[0] + delta_w_p_c2[1] + delta_w_p_c2[2] + delta_w_p_c2[3] + delta_w_p_rem[2] );
			    delta_w_c3 = ( delta_w_p_c3[0] + delta_w_p_c3[1] + delta_w_p_c3[2] + delta_w_p_c3[3] + delta_w_p_rem[3] );

			    delta_ws_p = _mm256_set_pd(delta_w_c3, delta_w_c2, delta_w_c1, delta_w_c0);

			    // -----------------------------------------------filter weight update--------------------------------------------

			    // modifying delta_w to account for sign function applied on weights
			    weights_p = _mm256_loadu_pd( (fil_w->data) + ind_fil_w(f, r, c) );

			    vmask_ones  = _mm256_cmp_pd(weights_p, ones_p , 0x02); //LE
			    vmask_mones = _mm256_cmp_pd(weights_p, mones_p, 0x0d); //GE
			    vmask_final = _mm256_and_pd(vmask_ones, vmask_mones); // if >= -1.00 && <= 1.00

			    alphas_p    = _mm256_set1_pd(alphas[f]);

			    if_result_p = _mm256_add_pd( _mm256_mul_pd(alphas_p, weights_p), recip_n_p);
			    ...
			    for (int b = 0; b < BATCH_SIZE; ++b)
			    {
	    			...
	    			for (int i = 0; i < N_ROWS_CONV; ++i)
	    			{
	    				...
	    				for (j = 0; j+3 < N_COLS_CONV; j=j+4)
	    				{
	    					...
	    					vmask = _mm256_cmp_pd(conv_val_p, zeroes_p, 0x0e); // 0x0e => GT.
	    					and_p = _mm256_and_pd(del_conv_val_p, vmask); // zero out the del_convs for which convs are less than equal to zero
	    					mul_p = _mm256_mul_pd(and_p, input_pixel_p);
	    					delta_w_p = _mm256_add_pd(delta_w_p, mul_p);
	    				}
...
// Vectorized: conv cols only, not filters
/*void bin_update_conv_weights(tensor* fil_w, tensor* fil_bin_w, double alphas[NUM_FILS], tensor* del_conv, tensor* conv_t, 
								tensor* input_images, int base, int shuffle_index[])
{
	INCREMENT_FLOPS(1)
	...
	double del_conv_val_f0;
	double del_conv_val_f1;
	double del_conv_val_f2;
	...
	__m256d conv_val_p_f0;
	__m256d conv_val_p_f2;
	__m256d conv_val_p_f1;

	__m256d del_conv_val_p_f0;
	__m256d del_conv_val_p_f1;
	__m256d del_conv_val_p_f2;
	...
	__m256d delta_w_p_f0;
	__m256d delta_w_p_f1;
	__m256d delta_w_p_f2;

    for (f = 0; f < NUM_FILS; ++f)
	{
		for (int r = 0; r < FIL_ROWS; ++r)
		{
			for (int c = 0; c < FIL_COLS; ++c)
			{
			    ...
			    delta_w_p_f0 = _mm256_set1_pd(0.0);
			    ...
			    for (int b = 0; b < BATCH_SIZE; ++b)
			    {
	    			...
	    			for (i = 0; i < N_ROWS_CONV; ++i)
	    			{
	    				for (j = 0; j+3 < N_COLS_CONV; j=j+4)
	    				{
	    					INCREMENT_FLOPS(36)

	    					conv_val_p_f0     = _mm256_loadu_pd( (conv_t->data)       + ind_conv_out(b, f  , i, j) );
	    					del_conv_val_p_f0 = _mm256_loadu_pd( (del_conv->data)     + ind_conv_out(b, f  , i, j) );

	    					input_pixel_p  = _mm256_loadu_pd( (input_images->data) + ind_input_img(cur_image, i+r, j+c) );


	    					// applying ReLU
	    					vmask_f0     = _mm256_cmp_pd(conv_val_p_f0, zeroes_p, 0x0e); // 0x0e => GT. 

	    					and_p_f0     = _mm256_and_pd(del_conv_val_p_f0, vmask_f0); // zero out the del_convs for which convs are less than equal to zero

	    					mul_p_f0     = _mm256_mul_pd(and_p_f0, input_pixel_p);

	    					delta_w_p_f0 = _mm256_add_pd(delta_w_p_f0, mul_p_f0);
	    				}
	    				...
	    				for (; j < N_COLS_CONV; ++j)
	    				{
	    					...
	    					conv_val_f0     =   (conv_t->data)[ind_conv_out(b, f  , i, j)];

	    					del_conv_val_f0 = (del_conv->data)[ind_conv_out(b, f  , i, j)];
	    					...
	    					if( conv_val > 0.0 )
	    					{
	    						delta_w_f0 += del_conv_val_f0 * input_pixel;
	    					}
	    				}
	    			}
			    }
			    ...
			    delta_w_f0 += ( delta_w_p_f0[0] + delta_w_p_f0[1] + delta_w_p_f0[2] + delta_w_p_f0[3] );


			    // modifying delta_w to account for sign function applied on weights
			    weight_f0 = (fil_w->data)[ind_fil_w(f, r, c)];

			    // -----------------------------------------------filter 0--------------------------------------------
			    if (weight_f0 <= 1.0 && weight_f0 >= -1.0)
			    {
			    	delta_w_f0 *= ( (alphas[f]*weight_f0) +  recip_n );
			    }
			    else
		    	{
		    		delta_w_f0 *= recip_n;
			    ...
			    (fil_w->data)[ind_fil_w(f, r, c)] = weight_f0 - MULTIPLIER * delta_w_f0;	            
			}
		}
	}
}*/

// Vectorized: filters, conv cols only;
/*void bin_update_conv_weights(tensor* fil_w, tensor* fil_bin_w, double alphas[NUM_FILS], tensor* del_conv, tensor* conv_t, 
								tensor* input_images, int base, int shuffle_index[])
{
	INCREMENT_FLOPS(1)
    ...
    for (f = 0; f+2 < NUM_FILS; f=f+3)
	{
		for (int r = 0; r < FIL_ROWS; ++r)
		{
			for (int c = 0; c < FIL_COLS; ++c)
			{
			    ...
			    delta_w_p_f0 = _mm256_set1_pd(0.0);
			    delta_w_p_f1 = _mm256_set1_pd(0.0);
			    delta_w_p_f2 = _mm256_set1_pd(0.0);
			    ...
			    for (int b = 0; b < BATCH_SIZE; ++b)
			    {
	    			...
	    			for (i = 0; i < N_ROWS_CONV; ++i)
	    			{
	    				for (j = 0; j+3 < N_COLS_CONV; j=j+4)
	    				{
	    					...
	    					conv_val_p_f0     = _mm256_loadu_pd( (conv_t->data)       + ind_conv_out(b, f  , i, j) );
	    					conv_val_p_f1     = _mm256_loadu_pd( (conv_t->data)       + ind_conv_out(b, f+1, i, j) );
	    					conv_val_p_f2     = _mm256_loadu_pd( (conv_t->data)       + ind_conv_out(b, f+2, i, j) );

	    					del_conv_val_p_f0 = _mm256_loadu_pd( (del_conv->data)     + ind_conv_out(b, f  , i, j) );
	    					del_conv_val_p_f1 = _mm256_loadu_pd( (del_conv->data)     + ind_conv_out(b, f+1, i, j) );
	    					del_conv_val_p_f2 = _mm256_loadu_pd( (del_conv->data)     + ind_conv_out(b, f+2, i, j) );
	    					...
	    					// applying ReLU
	    					vmask_f0     = _mm256_cmp_pd(conv_val_p_f0, zeroes_p, 0x0e); // 0x0e => GT. 
	    					vmask_f1     = _mm256_cmp_pd(conv_val_p_f1, zeroes_p, 0x0e);
	    					vmask_f2     = _mm256_cmp_pd(conv_val_p_f2, zeroes_p, 0x0e);

	    					and_p_f0     = _mm256_and_pd(del_conv_val_p_f0, vmask_f0); // zero out the del_convs for which convs are less than equal to zero
	    					and_p_f1     = _mm256_and_pd(del_conv_val_p_f1, vmask_f1);
	    					and_p_f2     = _mm256_and_pd(del_conv_val_p_f2, vmask_f2);

	    					mul_p_f0     = _mm256_mul_pd(and_p_f0, input_pixel_p);
	    					mul_p_f1     = _mm256_mul_pd(and_p_f1, input_pixel_p);
	    					mul_p_f2     = _mm256_mul_pd(and_p_f2, input_pixel_p);

	    					delta_w_p_f0 = _mm256_add_pd(delta_w_p_f0, mul_p_f0);
	    					delta_w_p_f1 = _mm256_add_pd(delta_w_p_f1, mul_p_f1);
	    					delta_w_p_f2 = _mm256_add_pd(delta_w_p_f2, mul_p_f2);
	    				}
	    				...
	    				for (; j < N_COLS_CONV; ++j)
	    				{
	    					...
	    					conv_val_f0     =   (conv_t->data)[ind_conv_out(b, f  , i, j)];
	    					conv_val_f1     =   (conv_t->data)[ind_conv_out(b, f+1, i, j)];
	    					conv_val_f2     =   (conv_t->data)[ind_conv_out(b, f+2, i, j)];

	    					del_conv_val_f0 = (del_conv->data)[ind_conv_out(b, f  , i, j)];
	    					del_conv_val_f1 = (del_conv->data)[ind_conv_out(b, f+1, i, j)];
	    					del_conv_val_f2 = (del_conv->data)[ind_conv_out(b, f+2, i, j)];
	    					...
	    					if( conv_val_f1 > 0.0 )
	    					{
	    						delta_w_f1 += del_conv_val_f1 * input_pixel;
	    					}

	    					if( conv_val_f2 > 0.0 )
	    					{
	    						delta_w_f2 += del_conv_val_f2 * input_pixel;
	    					}
	    				}
	    			}
			    }
			    ...
			    delta_w_f0 += ( delta_w_p_f0[0] + delta_w_p_f0[1] + delta_w_p_f0[2] + delta_w_p_f0[3] );
			    delta_w_f1 += ( delta_w_p_f1[0] + delta_w_p_f1[1] + delta_w_p_f1[2] + delta_w_p_f1[3] );
			    delta_w_f2 += ( delta_w_p_f2[0] + delta_w_p_f2[1] + delta_w_p_f2[2] + delta_w_p_f2[3] );


			    // modifying delta_w to account for sign function applied on weights
			    weight_f0 = (fil_w->data)[ind_fil_w(f  , r, c)];
			    weight_f1 = (fil_w->data)[ind_fil_w(f+1, r, c)];
			    weight_f2 = (fil_w->data)[ind_fil_w(f+2, r, c)];
		    	...
		    	// -----------------------------------------------filter 1--------------------------------------------
		    	if (weight_f1 <= 1.0 && weight_f1 >= -1.0)
			    {
			    	delta_w_f1 *= ( (alphas[f+1]*weight_f1) +  recip_n );
			    }
			    else
		    	{
		    		delta_w_f1 *= recip_n;
		    	...
		    	// -----------------------------------------------filter 2--------------------------------------------
		    	if (weight_f2 <= 1.0 && weight_f2 >= -1.0)
			    {
			    	delta_w_f2 *= ( (alphas[f+2]*weight_f2) +  recip_n );
			    }
			    else
		    	{
		    		delta_w_f2 *= recip_n;
			    ...
			    (fil_w->data)[ind_fil_w(f  , r, c)] = weight_f0 - MULTIPLIER * delta_w_f0;
			    (fil_w->data)[ind_fil_w(f+1, r, c)] = weight_f1 - MULTIPLIER * delta_w_f1;
			    (fil_w->data)[ind_fil_w(f+2, r, c)] = weight_f2 - MULTIPLIER * delta_w_f2;
	...
	// leftover filters at the end, after dividing number of filters by 3
	for (; f < NUM_FILS; ++f)
	{
		for (int r = 0; r < FIL_ROWS; ++r)
		{
...
// Vectorized: filters, conv rows and cols; did not improve perf
/*void bin_update_conv_weights(tensor* fil_w, tensor* fil_bin_w, double alphas[NUM_FILS], tensor* del_conv, tensor* conv_t, 
								tensor* input_images, int base, int shuffle_index[])
{
	INCREMENT_FLOPS(1)
	...
	__m256d conv_val_p_i0_f0, conv_val_p_i1_f0, conv_val_p_i2_f0, conv_val_p_i3_f0;
	__m256d conv_val_p_i0_f2, conv_val_p_i1_f2, conv_val_p_i2_f2, conv_val_p_i3_f2;
	__m256d conv_val_p_i0_f1, conv_val_p_i1_f1, conv_val_p_i2_f1, conv_val_p_i3_f1;

	__m256d del_conv_val_p_i0_f0, del_conv_val_p_i1_f0, del_conv_val_p_i2_f0, del_conv_val_p_i3_f0;
	__m256d del_conv_val_p_i0_f1, del_conv_val_p_i1_f1, del_conv_val_p_i2_f1, del_conv_val_p_i3_f1;
	__m256d del_conv_val_p_i0_f2, del_conv_val_p_i1_f2, del_conv_val_p_i2_f2, del_conv_val_p_i3_f2;

	__m256d vmask_i0_f0, vmask_i1_f0, vmask_i2_f0, vmask_i3_f0;
	__m256d vmask_i0_f1, vmask_i1_f1, vmask_i2_f1, vmask_i3_f1;
	__m256d vmask_i0_f2, vmask_i1_f2, vmask_i2_f2, vmask_i3_f2;

	__m256d and_p_i0_f0, and_p_i1_f0, and_p_i2_f0, and_p_i3_f0;
	__m256d and_p_i0_f1, and_p_i1_f1, and_p_i2_f1, and_p_i3_f1;
	__m256d and_p_i0_f2, and_p_i1_f2, and_p_i2_f2, and_p_i3_f2;

	__m256d mul_p_i0_f0, mul_p_i1_f0, mul_p_i2_f0, mul_p_i3_f0;
	__m256d mul_p_i0_f1, mul_p_i1_f1, mul_p_i2_f1, mul_p_i3_f1;
	__m256d mul_p_i0_f2, mul_p_i1_f2, mul_p_i2_f2, mul_p_i3_f2;

	__m256d delta_w_p_i0_f0, delta_w_p_i1_f0, delta_w_p_i2_f0, delta_w_p_i3_f0;
	__m256d delta_w_p_i0_f1, delta_w_p_i1_f1, delta_w_p_i2_f1, delta_w_p_i3_f1;
	__m256d delta_w_p_i0_f2, delta_w_p_i1_f2, delta_w_p_i2_f2, delta_w_p_i3_f2;

	__m256d delta_w_p_temp0_f0, delta_w_p_temp1_f0, delta_w_p_temp2_f0, delta_w_p_temp3_f0;
	__m256d delta_w_p_temp0_f1, delta_w_p_temp1_f1, delta_w_p_temp2_f1, delta_w_p_temp3_f1;
	__m256d delta_w_p_temp0_f2, delta_w_p_temp1_f2, delta_w_p_temp2_f2, delta_w_p_temp3_f2;

	__m256d input_pixel_p;

	__m256d input_pixel_i0_p, input_pixel_i1_p, input_pixel_i2_p, input_pixel_i3_p;
    ...
    for (f = 0; f+2 < NUM_FILS; f=f+3)
	{
		for (int r = 0; r < FIL_ROWS; ++r)
		{
			for (int c = 0; c < FIL_COLS; ++c)
			{
				...
				delta_w_p_i0_f0 = _mm256_set1_pd(0.0), delta_w_p_i1_f0 = _mm256_set1_pd(0.0), delta_w_p_i2_f0 = _mm256_set1_pd(0.0), delta_w_p_i3_f0 = _mm256_set1_pd(0.0);
				delta_w_p_i0_f1 = _mm256_set1_pd(0.0), delta_w_p_i1_f1 = _mm256_set1_pd(0.0), delta_w_p_i2_f1 = _mm256_set1_pd(0.0), delta_w_p_i3_f1 = _mm256_set1_pd(0.0);
				delta_w_p_i0_f2 = _mm256_set1_pd(0.0), delta_w_p_i1_f2 = _mm256_set1_pd(0.0), delta_w_p_i2_f2 = _mm256_set1_pd(0.0), delta_w_p_i3_f2 = _mm256_set1_pd(0.0);
			    ...
			    for (int b = 0; b < BATCH_SIZE; ++b)
			    {
	    			...
	    			for (i = 0; i+3 < N_ROWS_CONV; i=i+4)
	    			{
	    				for (j = 0; j+3 < N_COLS_CONV; j=j+4)
	    				{
	    					INCREMENT_FLOPS(36)

	    					conv_val_p_i0_f0     = _mm256_loadu_pd( (conv_t->data)       + ind_conv_out(b, f  , i  , j) );
	    					conv_val_p_i0_f1     = _mm256_loadu_pd( (conv_t->data)       + ind_conv_out(b, f+1, i  , j) );
	    					conv_val_p_i0_f2     = _mm256_loadu_pd( (conv_t->data)       + ind_conv_out(b, f+2, i  , j) );

	    					conv_val_p_i1_f0     = _mm256_loadu_pd( (conv_t->data)       + ind_conv_out(b, f  , i+1, j) );
							conv_val_p_i1_f1     = _mm256_loadu_pd( (conv_t->data)       + ind_conv_out(b, f+1, i+1, j) );
							conv_val_p_i1_f2     = _mm256_loadu_pd( (conv_t->data)       + ind_conv_out(b, f+2, i+1, j) );

							conv_val_p_i2_f0     = _mm256_loadu_pd( (conv_t->data)       + ind_conv_out(b, f  , i+2, j) );
							conv_val_p_i2_f1     = _mm256_loadu_pd( (conv_t->data)       + ind_conv_out(b, f+1, i+2, j) );
							conv_val_p_i2_f2     = _mm256_loadu_pd( (conv_t->data)       + ind_conv_out(b, f+2, i+2, j) );

							conv_val_p_i3_f0     = _mm256_loadu_pd( (conv_t->data)       + ind_conv_out(b, f  , i+3, j) );
							conv_val_p_i3_f1     = _mm256_loadu_pd( (conv_t->data)       + ind_conv_out(b, f+1, i+3, j) );
							conv_val_p_i3_f2     = _mm256_loadu_pd( (conv_t->data)       + ind_conv_out(b, f+2, i+3, j) );




	    					del_conv_val_p_i0_f0 = _mm256_loadu_pd( (del_conv->data)     + ind_conv_out(b, f  , i  , j) );
	    					del_conv_val_p_i0_f1 = _mm256_loadu_pd( (del_conv->data)     + ind_conv_out(b, f+1, i  , j) );
	    					del_conv_val_p_i0_f2 = _mm256_loadu_pd( (del_conv->data)     + ind_conv_out(b, f+2, i  , j) );

	    					del_conv_val_p_i1_f0 = _mm256_loadu_pd( (del_conv->data)     + ind_conv_out(b, f  , i+1, j) );
	    					del_conv_val_p_i1_f1 = _mm256_loadu_pd( (del_conv->data)     + ind_conv_out(b, f+1, i+1, j) );
	    					del_conv_val_p_i1_f2 = _mm256_loadu_pd( (del_conv->data)     + ind_conv_out(b, f+2, i+1, j) );

	    					del_conv_val_p_i2_f0 = _mm256_loadu_pd( (del_conv->data)     + ind_conv_out(b, f  , i+2, j) );
	    					del_conv_val_p_i2_f1 = _mm256_loadu_pd( (del_conv->data)     + ind_conv_out(b, f+1, i+2, j) );
	    					del_conv_val_p_i2_f2 = _mm256_loadu_pd( (del_conv->data)     + ind_conv_out(b, f+2, i+2, j) );

	    					del_conv_val_p_i3_f0 = _mm256_loadu_pd( (del_conv->data)     + ind_conv_out(b, f  , i+3, j) );
	    					del_conv_val_p_i3_f1 = _mm256_loadu_pd( (del_conv->data)     + ind_conv_out(b, f+1, i+3, j) );
	    					del_conv_val_p_i3_f2 = _mm256_loadu_pd( (del_conv->data)     + ind_conv_out(b, f+2, i+3, j) );



	    					input_pixel_i0_p  = _mm256_loadu_pd( (input_images->data) + ind_input_img(cur_image, i  +r, j+c) );
	    					input_pixel_i1_p  = _mm256_loadu_pd( (input_images->data) + ind_input_img(cur_image, i+1+r, j+c) );
	    					input_pixel_i2_p  = _mm256_loadu_pd( (input_images->data) + ind_input_img(cur_image, i+2+r, j+c) );
	    					input_pixel_i3_p  = _mm256_loadu_pd( (input_images->data) + ind_input_img(cur_image, i+3+r, j+c) );


	    					// -----------------------------------------i th row-----------------------------------------------
	    					// applying ReLU
	    					    vmask_i0_f0 = _mm256_cmp_pd(    conv_val_p_i0_f0, zeroes_p, 0x0e); // 0x0e => GT. 
	    					    vmask_i0_f1 = _mm256_cmp_pd(    conv_val_p_i0_f1, zeroes_p, 0x0e);
	    					    vmask_i0_f2 = _mm256_cmp_pd(    conv_val_p_i0_f2, zeroes_p, 0x0e);

	    					    and_p_i0_f0 = _mm256_and_pd(del_conv_val_p_i0_f0,      vmask_i0_f0); // zero out the del_convs for which convs are less than equal to zero
	    					    and_p_i0_f1 = _mm256_and_pd(del_conv_val_p_i0_f1,      vmask_i0_f1);
	    					    and_p_i0_f2 = _mm256_and_pd(del_conv_val_p_i0_f2,      vmask_i0_f2);

	    					    mul_p_i0_f0 = _mm256_mul_pd(         and_p_i0_f0, input_pixel_i0_p);
	    					    mul_p_i0_f1 = _mm256_mul_pd(         and_p_i0_f1, input_pixel_i0_p);
	    					    mul_p_i0_f2 = _mm256_mul_pd(         and_p_i0_f2, input_pixel_i0_p);

	    					delta_w_p_i0_f0 = _mm256_add_pd(     delta_w_p_i0_f0,      mul_p_i0_f0);
	    					delta_w_p_i0_f1 = _mm256_add_pd(     delta_w_p_i0_f1,      mul_p_i0_f1);
	    					delta_w_p_i0_f2 = _mm256_add_pd(     delta_w_p_i0_f2,      mul_p_i0_f2);


	    					// -----------------------------------------i+1 th row-----------------------------------------------
	    					// applying ReLU
	    					    vmask_i1_f0 = _mm256_cmp_pd(    conv_val_p_i1_f0, zeroes_p, 0x0e); // 0x0e => GT. 
	    					    vmask_i1_f1 = _mm256_cmp_pd(    conv_val_p_i1_f1, zeroes_p, 0x0e);
	    					    vmask_i1_f2 = _mm256_cmp_pd(    conv_val_p_i1_f2, zeroes_p, 0x0e);

	    					    and_p_i1_f0 = _mm256_and_pd(del_conv_val_p_i1_f0,      vmask_i1_f0); // zero out the del_convs for which convs are less than equal to zero
	    					    and_p_i1_f1 = _mm256_and_pd(del_conv_val_p_i1_f1,      vmask_i1_f1);
	    					    and_p_i1_f2 = _mm256_and_pd(del_conv_val_p_i1_f2,      vmask_i1_f2);

	    					    mul_p_i1_f0 = _mm256_mul_pd(         and_p_i1_f0, input_pixel_i1_p);
	    					    mul_p_i1_f1 = _mm256_mul_pd(         and_p_i1_f1, input_pixel_i1_p);
	    					    mul_p_i1_f2 = _mm256_mul_pd(         and_p_i1_f2, input_pixel_i1_p);

	    					delta_w_p_i1_f0 = _mm256_add_pd(     delta_w_p_i1_f0,      mul_p_i1_f0);
	    					delta_w_p_i1_f1 = _mm256_add_pd(     delta_w_p_i1_f1,      mul_p_i1_f1);
	    					delta_w_p_i1_f2 = _mm256_add_pd(     delta_w_p_i1_f2,      mul_p_i1_f2);


	    					// -----------------------------------------i+2 th row-----------------------------------------------
	    					// applying ReLU
	    					    vmask_i2_f0 = _mm256_cmp_pd(    conv_val_p_i2_f0, zeroes_p, 0x0e); // 0x0e => GT. 
	    					    vmask_i2_f1 = _mm256_cmp_pd(    conv_val_p_i2_f1, zeroes_p, 0x0e);
	    					    vmask_i2_f2 = _mm256_cmp_pd(    conv_val_p_i2_f2, zeroes_p, 0x0e);

	    					    and_p_i2_f0 = _mm256_and_pd(del_conv_val_p_i2_f0,      vmask_i2_f0); // zero out the del_convs for which convs are less than equal to zero
	    					    and_p_i2_f1 = _mm256_and_pd(del_conv_val_p_i2_f1,      vmask_i2_f1);
	    					    and_p_i2_f2 = _mm256_and_pd(del_conv_val_p_i2_f2,      vmask_i2_f2);

	    					    mul_p_i2_f0 = _mm256_mul_pd(         and_p_i2_f0, input_pixel_i2_p);
	    					    mul_p_i2_f1 = _mm256_mul_pd(         and_p_i2_f1, input_pixel_i2_p);
	    					    mul_p_i2_f2 = _mm256_mul_pd(         and_p_i2_f2, input_pixel_i2_p);

	    					delta_w_p_i2_f0 = _mm256_add_pd(     delta_w_p_i2_f0,      mul_p_i2_f0);
	    					delta_w_p_i2_f1 = _mm256_add_pd(     delta_w_p_i2_f1,      mul_p_i2_f1);
	    					delta_w_p_i2_f2 = _mm256_add_pd(     delta_w_p_i2_f2,      mul_p_i2_f2);


	    					// -----------------------------------------i th row-----------------------------------------------
	    					// applying ReLU
	    					    vmask_i3_f0 = _mm256_cmp_pd(    conv_val_p_i3_f0, zeroes_p, 0x0e); // 0x0e => GT. 
	    					    vmask_i3_f1 = _mm256_cmp_pd(    conv_val_p_i3_f1, zeroes_p, 0x0e);
	    					    vmask_i3_f2 = _mm256_cmp_pd(    conv_val_p_i3_f2, zeroes_p, 0x0e);

	    					    and_p_i3_f0 = _mm256_and_pd(del_conv_val_p_i3_f0,      vmask_i3_f0); // zero out the del_convs for which convs are less than equal to zero
	    					    and_p_i3_f1 = _mm256_and_pd(del_conv_val_p_i3_f1,      vmask_i3_f1);
	    					    and_p_i3_f2 = _mm256_and_pd(del_conv_val_p_i3_f2,      vmask_i3_f2);

	    					    mul_p_i3_f0 = _mm256_mul_pd(         and_p_i3_f0, input_pixel_i3_p);
	    					    mul_p_i3_f1 = _mm256_mul_pd(         and_p_i3_f1, input_pixel_i3_p);
	    					    mul_p_i3_f2 = _mm256_mul_pd(         and_p_i3_f2, input_pixel_i3_p);

	    					delta_w_p_i3_f0 = _mm256_add_pd(     delta_w_p_i3_f0,      mul_p_i3_f0);
	    					delta_w_p_i3_f1 = _mm256_add_pd(     delta_w_p_i3_f1,      mul_p_i3_f1);
	    					delta_w_p_i3_f2 = _mm256_add_pd(     delta_w_p_i3_f2,      mul_p_i3_f2);
			    ...
			    // ----------------------------------------------------filter 0------------------------------------------------
			    delta_w_p_temp0_f0 = _mm256_add_pd(   delta_w_p_i0_f0,    delta_w_p_i1_f0);
			    delta_w_p_temp1_f0 = _mm256_add_pd(   delta_w_p_i2_f0,    delta_w_p_i3_f0);
			    delta_w_p_temp2_f0 = _mm256_add_pd(delta_w_p_temp0_f0, delta_w_p_temp1_f0);

			    delta_w_p_temp3_f0 = _mm256_add_pd(      delta_w_p_f0, delta_w_p_temp2_f0);

			            delta_w_f0 += ( delta_w_p_temp3_f0[0] + delta_w_p_temp3_f0[1] + delta_w_p_temp3_f0[2] + delta_w_p_temp3_f0[3] );


	            // ----------------------------------------------------filter 1------------------------------------------------
			    delta_w_p_temp0_f1 = _mm256_add_pd(   delta_w_p_i0_f1,    delta_w_p_i1_f1);
			    delta_w_p_temp1_f1 = _mm256_add_pd(   delta_w_p_i2_f1,    delta_w_p_i3_f1);
			    delta_w_p_temp2_f1 = _mm256_add_pd(delta_w_p_temp0_f1, delta_w_p_temp1_f1);

			    delta_w_p_temp3_f1 = _mm256_add_pd(      delta_w_p_f1, delta_w_p_temp2_f1);

			            delta_w_f1 += ( delta_w_p_temp3_f1[0] + delta_w_p_temp3_f1[1] + delta_w_p_temp3_f1[2] + delta_w_p_temp3_f1[3] );


	            // ----------------------------------------------------filter 2------------------------------------------------
			    delta_w_p_temp0_f2 = _mm256_add_pd(   delta_w_p_i0_f2,    delta_w_p_i1_f2);
			    delta_w_p_temp1_f2 = _mm256_add_pd(   delta_w_p_i2_f2,    delta_w_p_i3_f2);
			    delta_w_p_temp2_f2 = _mm256_add_pd(delta_w_p_temp0_f2, delta_w_p_temp1_f2);

			    delta_w_p_temp3_f2 = _mm256_add_pd(      delta_w_p_f2, delta_w_p_temp2_f2);

			            delta_w_f2 += ( delta_w_p_temp3_f2[0] + delta_w_p_temp3_f2[1] + delta_w_p_temp3_f2[2] + delta_w_p_temp3_f2[3] );
